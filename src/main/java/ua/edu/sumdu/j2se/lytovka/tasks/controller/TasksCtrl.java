        ...
        while (lChkRunTask) {
            try {
                ...
                for (Task t : model) {
                    ...
                    log.debug("ВрТекущие dCur " + dToC(dCur));
                    log.debug("ВрПроверки dChek " + dToC(dChek));
                    log.debug("Вр_nextTimeAfter(dCur) dTime " + dToC(dTime));
                    ...
                    if (dTime != null) {
                        ...
                        log.debug("Math.abs(nChek - nTime)->nCtrlInt " + nCtrlInt);
